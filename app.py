...
def parse_document(ocr_text):
    ...
    # BBBEE / Unemployment Affidavit (Targeting "Full name:" table row & opening line)
    if "bbbe certification" in text_lower or "affidavit to confirm unemployment" in text_lower or "unemployment" in text_lower:
        ...
        return name or "Unknown_Candidate", id_num or "NoID", "BBBEE-Unemployment-Affidavit"
    ...
    # Criminal Check Affidavit
        ...
        return name or "Unknown_Candidate", id_num or "NoID", "Criminal-Check-Affidavit"
    ...
    # Smart ID
        ...
        id_match = re.search(r'\b\d{13}\b', ocr_text)
        fullname = f"{names}_{surname}" if names and surname else (names or surname or "Unknown_Candidate")
        return fullname, (id_match.group(0) if id_match else "NoID"), "Smart-ID"

    # Fallback
    id_match = re.search(r'\b\d{13}\b', ocr_text)
    return "Candidate_Doc", (id_match.group(0) if id_match else "NoID"), "Document"
                ...
                for uploaded_file in uploaded_files:
                    ...
                    if uploaded_file.type == "application/pdf":
                        ...
                        for page_idx in range(total_pages):
                            ...
                            new_filename = f"{name}_{id_number}_{doc_type}{page_suffix}.pdf"
                        ...
                        new_filename = f"{name}_{id_number}_{doc_type}{ext}"
